...
uint8_t XN297L_WriteReg(uint8_t reg, uint8_t value)
{
    uint8_t reg_val;
    XN297L_CSN_LOW;
    XN297L_SPI_TxRxByte(reg);
    reg_val = XN297L_SPI_TxRxByte(value);
    XN297L_CSN_HIGH;
    return reg_val;
}
...
uint8_t XN297L_ReadReg(uint8_t reg)
{
    ...
    XN297L_CSN_LOW;
    XN297L_SPI_TxRxByte(reg);
    reg_val = XN297L_SPI_TxRxByte(XN297L_CMD_NOP);
    XN297L_CSN_HIGH;
    return reg_val;
}
...
void XN297L_ReadToBuf(uint8_t reg, uint8_t *pBuf, uint8_t len)
{
    ...
    for (ctr = 0; ctr < len; ctr++)
    {
        pBuf[ctr] = XN297L_SPI_TxRxByte(XN297L_CMD_NOP);
    }
    XN297L_CSN_HIGH;
}
